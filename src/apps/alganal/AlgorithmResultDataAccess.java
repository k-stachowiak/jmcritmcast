...
public class AlgorithmResultDataAccess {
	...
	public static AlgorithmExperimentValues selectResultForCase(
		...
		try (PreparedStatement prStatement = connection
				.prepareStatement("SELECT first_costs, success_count "
						...
						+ "graph_index = ? AND constraint_base = ? AND "
						+ "tree_finder_type = ?")) {
			...
			prStatement.setDouble(6, experimentCase.getConstraintBase());
			prStatement.setString(7, experimentCase.getTreeFinderType()
					.toString());
	...
	public static void insert(Connection connection,
		...
		try (PreparedStatement prStatement = connection
			...
			prStatement.setString(7, experimentCase.getTreeFinderType()
					.toString());
			prStatement.setString(8, CommonDataAccess
					.costListToString(experimentValues.getFirstCosts()));
			prStatement.setDouble(9, experimentValues.getSuccessCount());
	...
	public static void update(Connection connection,
		...
		try (PreparedStatement prStatement = connection
			...
			prStatement.setDouble(8, experimentCase.getConstraintBase());
			prStatement.setString(9, experimentCase.getTreeFinderType()
					.toString());
