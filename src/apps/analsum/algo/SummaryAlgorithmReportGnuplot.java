...
public class SummaryAlgorithmReportGnuplot extends SummaryAlgorithmReportTemplate {
	...
	protected void onInit(TopologyType topologyType, int nodesCount, NodeGroupperType nodeGroupperType,
		...
		yLabel = attributeSelectors.get(0).getName();
		...
		inGroup = attributeSelectors.size();
	}

	@Override
	protected void onDataHeader(SummaryAlgorithmResultAttributeSelector attributeSelector,
			TreeFinderType treeFinderType) {
		dataHeaders.add(String.format("\"%s_{%s}\"", treeFinderType.toString(), attributeSelector.getName()));
